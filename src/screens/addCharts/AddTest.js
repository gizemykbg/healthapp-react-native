...
const AddTest = () => {
    ...
    //test
    const testHandleChange = (i, text, valueName) => {
        const Newtest = [...test];
    ...
    const testButton = () => {
        let newtest = [];
       test.map((val, index) => {
            newtest.push({ testName: val.value1, testValue: val.value2, id: val.index })
        }
        )
    ...
    return (
        <View style={styles.container}>
            ...
            <ScrollView style={styles.main} showsVerticalScrollIndicator={false}>
                <Text style={{ fontWeight: "400", fontSize: 17, textAlign: 'auto', paddingBottom: 40, color: colors.dark }}>Değer takibi için değerleri aşağıdaki alana yazınız.</Text>
                <TouchableOpacity onPress={() => setShowTest(!showTest)} style={{ height: 40, backgroundColor: colors.lightBlue, justifyContent: 'center', alignItems: 'center', borderRadius: 8, marginBottom: 10 }}>
                    <Text style={{ fontSize: 16, fontWeight: "600", color: colors.darkBlue }}>Test Girmek için Tıklayınız</Text>
                </TouchableOpacity>
                {
                    showTest &&
                    <View style={{ backgroundColor: colors.lightBlue, borderRadius: 8, padding: 10 }}>
                        <View style={{ marginBottom: 20 }}>
                            <Text style={{ textAlign: 'center', fontWeight: "500", fontSize: 18, color: colors.dark }}>Test Sonucu</Text>
                            {
                                    ...
                                    return (
                                        <View key={index} style={{ flex: 1, flexDirection: 'row', borderBottomWidth: 2, padding: 5, borderBottomColor: colors.white }}>
                                            ...
                                            <View style={{ flex: 1, justifyContent: 'center', alignItems: 'flex-start', paddingLeft: 5 }}>
                                                <Text style={{ fontWeight: "500", fontSize: 14, color: colors.dark }}>{item.testValue}</Text>
                                            </View>
                            ...
                            <TextInputAddT
                                ...
                                onChangeText1={testHandleChange}
                                onChangeText2={testHandleChange}
                                onPress={testButton}
